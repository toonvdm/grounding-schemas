...
class CHMM(object):
    ...
    def update_T(self):
        """Update t/he transition matrix given the accumulated counts matrix."""
    ...
    def learn_em_T_given_E(self, x, a, E, n_iter=100, term_early=True):
        """Run EM training, keeping E deterministic and fixed, learning T"""
        sys.stdout.flush()
        convergence = []
        pbar = trange(n_iter, position=0)
        ...
        for it in pbar:
            ...
            updateC2(self.C, self.T, self.n_clones, mess_fwd, mess_bwd, x, a)
            # M
            self.update_T()
            convergence.append(-log2_lik.mean())
            pbar.set_postfix(train_bps=convergence[-1])
...
@nb.njit
def updateC(C, T, n_clones, mess_fwd, mess_bwd, x, a, pseudocount=0):
    ...
    for t in range(1, timesteps):
        ...
        # for ac in range(4):
        #     if ac != aij:
        #         C[ac, j_start:j_stop, i_start:i_stop] += pseudocount * q


@nb.njit
def updateC2(C, T, n_clones, mess_fwd, mess_bwd, x, a):
    C[:] = 0
    for t in range(1, len(x)):
        # at time t-1 -> t we go from observation i to observation j
        aij = a[t - 1]
        q = mess_fwd[t].reshape(-1, 1) * T[aij] * mess_bwd[t].reshape(1, -1)
        q /= q.sum()
        C[aij] += q
